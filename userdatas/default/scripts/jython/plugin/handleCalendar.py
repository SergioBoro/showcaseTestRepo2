...
class myHandler(DefaultHandler):
    def startElement(self, namespaceURI, lname, qname, attrs):
        ...
        global resultData
        if (qname == "metadata"):
            resultMetadata = u"{date: %s, minHours: %s, maxHours: %s, dateInterval: '%s', style: '%s'}" % (attrs.getValue('date'), attrs.getValue('minHours'), attrs.getValue('maxHours'), attrs.getValue('dateInterval'), attrs.getValue('style'))
        if (qname == "event"):
