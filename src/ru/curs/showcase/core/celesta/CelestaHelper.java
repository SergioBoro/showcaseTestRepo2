...
public class CelestaHelper<T> {
	 ...
	 *            тип возвращаемого результата.
	 */
	public CelestaHelper(final CompositeContext oContex,
			final Class<T> aResultType) {
		super();
		this.contex = oContex;
	...
	public T runPython(final String sProcName, final Object... additionalParams) {
		...
		try {
			PyObject result = Celesta.getInstance().runPython(userSID,
					procName, params);
			if (result == null) {
				return null;
			}
			...
			throw new CelestaWorkerException("Error run celesta jython script",
					ex);
		}
	}
	...
	protected Object[] mergeAddAndGeneralParameters(
		...
		if (additionalParams != null && additionalParams.length > 0) {
			resultParams = new Object[additionalParams.length
					+ LENGTH_GENERAL_PARAMS];
			System.arraycopy(additionalParams, 0, resultParams,
					LENGTH_GENERAL_PARAMS, additionalParams.length);
		} else {
		...
		resultParams[FILTER_CONTEXT_INDEX] = XMLUtils.convertXmlToJson(context
				.getFilter());
		resultParams[SESSION_CONTEXT_INDEX] = XMLUtils.convertXmlToJson(context
				.getSession());
		return resultParams;
	}
}
