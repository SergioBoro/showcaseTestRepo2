...
public class SelectorCelestaGateway implements SelectorGateway {
	...
	public ResultSelectorData getData(final DataRequest req) throws Exception {
		CompositeContext context = (CompositeContext) req.getAddData()
				.getContext();
		CelestaHelper<ResultSelectorData> helper = new CelestaHelper<ResultSelectorData>(
				context, ResultSelectorData.class);
		...
		if (procName.indexOf(Constants.PROCNAME_SEPARATOR) != -1) {
			...
			String procCount = procName.substring(0,
					procName.indexOf(Constants.PROCNAME_SEPARATOR));
			String procList = procName.substring(procName
					.indexOf(Constants.PROCNAME_SEPARATOR)
					+ Constants.PROCNAME_SEPARATOR.length());

			// получение кол-ва
			ResultSelectorData rsdCount = helper.runPython(procCount,
					req.getParams(), req.getCurValue(), req.isStartsWith());
			// получение данных
			ResultSelectorData rsdData = helper.runPython(procList,
					req.getParams(), req.getCurValue(), req.isStartsWith(),
					req.getFirstRecord(), req.getRecordCount());

			return new ResultSelectorData(rsdData.getDataRecordList(),
					rsdCount.getCount());

		} else {
			// единый скрипт получения данных и кол-во записей
			ResultSelectorData result = helper.runPython(procName,
					req.getParams(), req.getCurValue(), req.isStartsWith(),
					req.getFirstRecord(), req.getRecordCount());
			return result;
		}
