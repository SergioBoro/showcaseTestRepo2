 ...
 */
public class JSFilter2 extends DialogBoxWithCaptionButton {

	private final VerticalPanel p = new VerticalPanel();
	private final FlowPanel hpFooter = new FlowPanel();

	// private final StackLayoutPanel accordeon = new StackLayoutPanel(Unit.EM);

	private final StackPanel accordeon = new StackPanel();

	private final ScrollPanel sp = new ScrollPanel();

	private int conditionCount = 0;

	JSFilter2(final String data) {
		setText("Фильтр");

		p.setSize("100%", "100%");

		sp.setSize("700px", "420px");

		accordeon.setSize("100%", "400px");

		// accordeon.setSize("100%", "100%");
		hpFooter.setSize("100%", "100%");

		final int headerSize = 2;
		// accordeon.add(createFiltersItem(), new
		// HTML("OR (\"Регион\" содержит 'обл')"), headerSize);
		// accordeon
		// .add(createFiltersItem(), new
		// HTML("OR (\"Регион\" содержит 'моск')"), headerSize);
		// accordeon.add(createFiltersItem(), new
		// HTML("AND (10 <= \"4кв. 2005г.\" <= 700)"),
		// headerSize);

		accordeon.add(createFiltersItem(), "OR (\"Регион\" содержит 'обл')");
		accordeon.add(createFiltersItem(), "OR (\"Регион\" содержит 'моск')");
		accordeon.add(createFiltersItem(), "AND (10 <= \"4кв. 2005г.\" <= 700)");
		...
		Button btnAdd = new Button("Добавить условие", new ClickHandler() {
			...
			public void onClick(final ClickEvent event) {
				...
				// new HTML("Условие " + String.valueOf(conditionCount)),
				// headerSize);

				accordeon.add(createFiltersItem(), "Условие " + String.valueOf(conditionCount));
